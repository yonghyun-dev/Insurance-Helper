...
- **진행: Sprint 37 (2026-07-20)** — 팀 검토 갭 3건 완료(CI 게이트 · prompts/v1 버전관리 · 준비도 스코어 게이지). 제안서 항목 2/2 완료(재청구 논리 포함) + EXAONE 사유서(PM-42) + CI 픽스. pytest 1138
...
- **제안서 '구조화된 재청구 논리' (F-14)** ✅: next_steps 자유텍스트 → **미충족(gap) → 보완 행동(action)
  → 근거 조항(basis)** 구조화. LLM structured output 스키마에 reclaim 강제(strict) + `_build_reclaim` 방어
  조립(빈/불완전 항목 제거·식별자 스트립) + basis 는 인용 조항명만 허용(프롬프트 규칙 — prompts/v1 첫 실전 활용).
  프론트 "이렇게 보완하면 다시 확인할 수 있어요" 카드. 라이브 검증: 도수치료 모호 시나리오 → 미충족 3건
  각각 서류·행동 + 제3조/제4조 근거 구조화 확인(+6 tests).
- **EXAONE 미채택 사유서** ✅: `docs/pm/42_exaone-decision-rationale.md` — 검증 사실(접속·FC ✅, strict 출력 ◐,
  실측 열위) + 사유 4건(스키마 계약·멀티모달·OCR 정합·정확도) + 보고서용 문구 + 재도입 조건. 팀 보고서 첨부용.
- **CI 첫 실행 발견 픽스**: 골든셋 코퍼스 정합 테스트가 로컬 app.db(gitignore) 의존 → skipif 가드,
  freeform QA 테스트가 로컬 인덱스에 우연 의존(vectorstore 패치가 실경로 밖) → rag_service 경계 패치로 교체.
  app.db 제거 조건에서 CI 재현 통과.
- 게이트: pytest **1138** · ruff · tsc · build. 스프린트 37 제안서 항목 2/2 완료 — **제안서 미구현 0 달성**.
